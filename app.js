...
async function registerTrialUser() {
  ...
  catch (e) { console.error("registerTrialUser:", e); }
}
async function registerTrial() {
  try {
    if (acctJwt()) await acctApi("/api/billing/register-trial", { method: "POST" });
    else await api("/api/billing/register-trial", { method: "POST" });
  } catch (e) { console.error("registerTrial:", e); }
}
async function checkBillingStatus() {
  try {
    ...
    console.error("checkBillingStatus:", e);
    return true;
  }
}
function showBillingModal() { const m = $("#billing-modal"); if (m) m.classList.remove("hidden"); }
function hideBillingModal() { const m = $("#billing-modal"); if (m) m.classList.add("hidden"); }
...
async function createCheckoutLink() {
  try {
    ...
    console.error("createCheckoutLink:", e);
...
// Registro de conta + pareamento automático
async function acctRegister() {
  ...
  try {
    ...
    if (btnEl) { btnEl.disabled = false; btnEl.textContent = "Criar Conta"; }
  }
}
...
function createSplash() {
  ...
  lunaLogo.src = "lunapngcinza.png"; lunaLogo.alt = "Luna Logo"; lunaLogo.className = "splash-logo";
  ...
  helseniaLogo.src = "logohelsenia.png"; helseniaLogo.alt = "Helsenia Logo"; helseniaLogo.className = "splash-logo";
...
function hideSplash() {
  ...
  if (el) { el.classList.add("fade-out"); setTimeout(() => { el.remove(); }, 800); }
...
/* =========================================
 * 12) PREFETCH (nomes/últimas/classificação leve)
 * ======================================= */
        ...
        try {
          ...
          if (pvHit) {
            ...
            const data = await api("/api/messages", {
              method: "POST", body: JSON.stringify({ chatid, limit: 1, sort: "-messageTimestamp" }),
            });
...
async function openChat(ch) {
  ...
  if (known) upsertStagePill(known.stage);
  else {
...
async function getOrInitStage(chatid, { messages = [] } = {}) {
  ...
  try {
    ...
    if (one?.stage) { const rec = setStage(chatid, one.stage); return rec; }
  } catch {}
...
function pickMediaInfo(m) {
  ...
  const mime =
    m.mimetype || m.mime ||
    mm?.imageMessage?.mimetype || mm?.videoMessage?.mimetype || mm?.documentMessage?.mimetype ||
    mm?.audioMessage?.mimetype || (mm?.stickerMessage ? "image/webp" : "") || "";
  const url =
  ...
  const dataUrl =
    m.dataUrl || mm?.imageMessage?.dataUrl || mm?.videoMessage?.dataUrl || mm?.documentMessage?.dataUrl ||
    mm?.stickerMessage?.dataUrl || mm?.audioMessage?.dataUrl || "";
  const caption =
    m.caption || mm?.imageMessage?.caption || mm?.videoMessage?.caption || mm?.documentMessage?.caption ||
    mm?.documentMessage?.fileName || m.text || mm?.conversation || m.body || "";
...
function renderReplyPreview(container, m) {
  ...
  if (!qm) return;
  const qt =
    qm?.extendedTextMessage?.text || qm?.conversation || qm?.imageMessage?.caption ||
    qm?.videoMessage?.caption || qm?.documentMessage?.caption || qm?.text || "";
...
function renderInteractive(container, m) {
  ...
  if (listMsg) {
    ...
    if (listMsg.title) {
      ...
      h.style.fontWeight = "600"; h.style.marginBottom = "6px"; h.textContent = listMsg.title;
      card.appendChild(h);
    }
    if (listMsg.description) {
      const d = document.createElement("div");
      d.style.fontSize = "12px"; d.style.opacity = ".85"; d.style.marginBottom = "6px"; d.textContent = listMsg.description;
      card.appendChild(d);
    }
    (listMsg.sections || []).forEach((sec) => {
      if (sec.title) {
        const st = document.createElement("div");
        st.style.margin = "6px 0 4px"; st.style.fontSize = "12px"; st.style.opacity = ".8"; st.textContent = sec.title;
        card.appendChild(st);
      }
      (sec.rows || []).forEach((row) => {
        const opt = document.createElement("div");
        opt.style.padding = "6px 8px"; opt.style.border = "1px solid var(--muted,#eee)";
        opt.style.borderRadius = "6px"; opt.style.marginBottom = "6px";
  ...
  if (btnsMsg) {
    ...
    if (title) {
      const h = document.createElement("div");
      h.style.fontWeight = "600"; h.style.marginBottom = "6px"; h.textContent = title;
      card.appendChild(h);
    }
    if (text) {
      const d = document.createElement("div");
      d.style.fontSize = "12px"; d.style.opacity = ".85"; d.style.marginBottom = "6px"; d.textContent = text;
      card.appendChild(d);
    }
...
function isFromMe(m) {
  return !!(
    m?.fromMe || m?.fromme || m?.from_me || m?.key?.fromMe || m?.message?.key?.fromMe || m?.sender?.fromMe ||
...
function appendMessageBubble(pane, m) {
  ...
  const plainText =
    m.text || m.message?.text || m?.message?.extendedTextMessage?.text || m?.message?.conversation || m.caption || m.body || "";
  ...
  if (mime && /^image\/webp$/i.test(mime) && (url || dataUrl)) {
    const img = document.createElement("img");
    img.alt = "figurinha"; img.style.maxWidth = "160px"; img.style.borderRadius = "8px";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("#btn-logout") && ($("#btn-logout").onclick = () => { try { stopStream(); } catch {} localStorage.clear(); location.reload(); });
